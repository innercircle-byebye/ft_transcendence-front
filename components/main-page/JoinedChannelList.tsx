...
const JoinedChannelList = () => {
  ...
  return (
    <div className="rounded-xl bg-sky-700 text-center flex flex-col w-1/2">
      {/* title */}
      ...
      <div className="flex flex-col space-y-1 mx-4 py-2">
        {channelData?.map((item: IChannel) => (
          <button type="button" key={item.channelId} className="bg-amber-50 rounded-md w-full py-1 px-2 flex justify-between">
            <div>{`# ${item.name}`}</div>
            <div>count</div>
          </button>
        ))}

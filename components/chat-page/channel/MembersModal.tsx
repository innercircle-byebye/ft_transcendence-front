...
const MembersModal: VFC = () => {
  ...
  const onClickInviteGame = useCallback((inviteMemberNickname: string) => {
    router.push(`/play/create-room?invite=${inviteMemberNickname}`);
  }, [router]);
          ...
          {channelMemberData.map((data) => {
            ...
            return (
              <div key={data.userId} className="flex flex-row justify-between space-x-2">
                ...
                <button type="button" onClick={() => onClickInviteGame(data.user.nickname)} className="bg-blue-400 font-semibold text-sm rounded-full w-16 h-7">게임신청</button>

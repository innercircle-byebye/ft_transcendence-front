...
const CreateChannel = () => {
  ...
  const [inputPasswordType, setInputPasswordType] = useState({ type: 'password', visible: false });
  ...
  const onClickPasswordEye = useCallback(() => {
    setInputPasswordType((prev) => ({ type: prev.type === 'text' ? 'password' : 'text', visible: !prev.visible }));
  }, []);
  ...
  return (
    <div className="w-screen h-full flex flex-col items-center space-y-20">
      <div className="grid grid-cols-7 justify-items-start mt-20">
        ...
        <div className="col-start-2 col-span-6 text-5xl">Create Chat Channel</div>
      </div>

      <div className="grid grid-cols-2 gap-10 items-center">
        <input className="col-span-2 w-80 px-6 py-4 rounded-full bg-gray-100 text-xl" placeholder="채널명" type="text" value={channelName} onChange={onChangeChannelName} />
        <div className="ml-3 text-gray-700 font-medium">최대멤버수</div>
        <input className="px-6 py-2 w-24 rounded-full bg-gray-100 text-xl" type="number" min={3} value={maxMemberNum} onChange={onChangeMaxMemberNum} />
          ...
          && <>
            ...
            <div>
              <div className="flex flex-row items-center space-x-2">
                <input className="px-6 py-2 w-24 rounded-full bg-gray-100 text-lg" type={inputPasswordType.type} value={password} onChange={onChangePassword} placeholder="****" maxLength={4} />
                <span onClick={onClickPasswordEye}>
                  {inputPasswordType.visible ? <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
              ...
              {!isValidPassword && <div className="absolute items-center text-red-500 text-xs italic">비밀번호 숫자4자리 입력해주세요</div>}
            </div>
          </>
        }
      </div>
